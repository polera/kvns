...
fn main() {
    ...
    let t0 = std::thread::spawn(move || {
        tokio_uring::start(async move {
            let metrics_addr: SocketAddr = config0
                .metrics_listen_addr()
                .parse()
            ...
            let (backend, classic_store) = if config0.sharded_mode {
                if config0.persist_path.is_some() {
                    warn!("KVNS_PERSIST_PATH is ignored in sharded mode");
                }
                info!(shard_count = config0.shard_count, "experimental sharded mode enabled");
                (server::Backend::Sharded(sharded::ShardedDb::new(config0.memory_limit, config0.shard_count)), None)
            } else {
                let initial_db = match &config0.persist_path {
                    None => store::Db::new(config0.memory_limit),
                    Some(path) => {
                        let p = PathBuf::from(path);
                        match persist::load(&p, config0.memory_limit) {
                            Ok(db) => { let key_count: usize = db.entries.values().map(|ns| ns.len()).sum(); info!(path = %path, keys = key_count, "loaded store from disk"); db }
                            Err(e) if e.kind() == std::io::ErrorKind::NotFound => { info!(path = %path, "no existing store file, starting fresh"); store::Db::new(config0.memory_limit) }
                            Err(e) => { warn!(error = %e, path = %path, "failed to load store from disk, starting fresh"); store::Db::new(config0.memory_limit) }
                        }
                    }
                };
                let initial_db = initial_db.with_eviction(config0.eviction_threshold, config0.eviction_policy.clone(), config0.namespace_eviction_policies.clone());
                let store = Arc::new(RwLock::new(initial_db));
                if let Some(ref path) = config0.persist_path {
                    tokio::spawn(persist::run_periodic_flush(Arc::clone(&store), PathBuf::from(path), config0.persist_interval_secs));
                }
                if config0.eviction_policy == config::EvictionPolicy::ExpireAfterRead || config0.namespace_eviction_policies.values().any(|p| *p == config::EvictionPolicy::ExpireAfterRead) {
            ...
            let limits = server::ServerLimits {
                resp: resp::RespLimits {
                    max_array_len: config0.max_resp_args,
                    max_bulk_len: config0.max_resp_bulk_len,
                    max_inline_len: config0.max_resp_inline_len,
                },
            };
            ...
            // Final persist on shutdown (thread 0 only).
            if let (Some(path), Some(store)) = (persist_path.as_ref(), classic_store.as_ref()) {
